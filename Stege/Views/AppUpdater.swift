...
final class AppUpdater: ObservableObject {
    ...
    private func fallbackDownloadURL(for version: String) -> URL? {
        ...
        let urlString =
            "https://github.com/mocki-toki/barik/releases/download/\(version)/stege-v\(versionWithoutPrefix).zip"
    ...
    private func downloadAndUnzip(
        ...
        let downloadTask = URLSession.shared.downloadTask(with: assetURL) {
            ...
            do {
                ...
                let newAppURL = tempDir.appendingPathComponent("Stege.app")
                if fileManager.fileExists(atPath: newAppURL.path) {
                    DispatchQueue.main.async {
                        completion(tempDir)
                    }
                } else {
                    print("Unzipping failed: Stege.app not found in archive")
                    DispatchQueue.main.async {
                        completion(nil)
                    }
                }
    ...
    func installUpdate(latest version: String) {
        ...
        let newAppURL = URL(fileURLWithPath: downloadedPath)
            .appendingPathComponent("Stege.app")
        let destinationURL = URL(fileURLWithPath: "/Applications/Stege.app")
        let script = """
            #!/bin/bash
            sleep 2
